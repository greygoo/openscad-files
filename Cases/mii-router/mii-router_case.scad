...
rim_screws_c        = 1.5;  // space around case connection screws
mki                 = 2;    // value for rounding corners of cases

// battery values
dim_batt_board      = [99.1,29.15,1.5]; // dimension of the board itself (wo components)
uppers_batt         = 4; // space used for ocmponents on top of board
lowers_batt         = 20.5; // space used for components on bottom of board
...
cuts_batt           = [[[84,-2.5],[12,6.8],hole_depth,0], // USB2 port
...
cuts_pi             = [[[5,-6.2],[24.8,6.9],hole_depth,0],        // USB
                       [[44.25,-7.5],[17.5,7.6],hole_depth,0],  // HDMI
                       [[0.6,-9],[28.6,16],hole_depth+4.8,2],       // camera/cables
                       [[11.25,-6.8],[11.5,6.8],hole_depth,3]]; // SD card
...
dia_conn            = 3; // diameter of screws connectiong all cases
...
loc_conn_high       = [[-(rim_screws_c+dia_conn)/2+wall,
                        (rim_screws_c+dia_conn)/2],
                       [(rim_screws_c+dia_conn)/2,
                        dim_batt_board[1]+2*(wall+rim)+(rim_screws_c+dia_conn)/2-wall],
                       [dim_t3_board[0]+2*(wall+rim)+(rim_screws_c+dia_conn)/2,
                        -(dim_pi_board[1]-dim_batt_board[1])-(rim_screws_c+dia_conn)/2+wall],
                       [dim_t3_board[0]+2*(wall+rim)-(rim_screws_c+dia_conn)/2,
                        dim_batt_board[1]+2*(wall+rim)+(rim_screws_c+dia_conn)/2-wall]];
loc_conn_low        = [[dim_batt_board[0]+2*(wall+rim)+(rim_screws_c+dia_conn)/2-wall,
                        (rim_screws_c+dia_conn)/2],
                       [dim_batt_board[0]+2*(wall+rim)-(rim_screws_c+dia_conn)/2,
                        dim_batt_board[1]+2*(wall+rim)+(rim_screws_c+dia_conn)/2-wall]];
...
module complete_object(){
    difference(){
        union(){
            ...
            difference(){
                hull(){
                    connection_short(dia=dia_conn+rim_screws_c,fn=$fn);
                    connection_long(dia=dia_conn+rim_screws_c,fn=$fn);
